...
def process_docx_text(docx_file, skip_lists=True):
    ...
    if skip_lists:
        # Use a custom function to remove lists
...
# Function to translate text with a fallback mechanism
def translate_text_with_fallback(text, target_language):
    try:
        return translate_text(text, target_language)
    except Exception as e:
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if text is not None:
            ...
            target_language = st.selectbox("Select Language to Translate:", list(language_mapping.values()))
